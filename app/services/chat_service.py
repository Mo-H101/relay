...
class ChatService:
    ...
    def _fallback_reason(
        ...
        Returns the reason of the most recent failed attempt, or None
        when the first candidate won.
        """
        ...
        if provider.name == first_provider and model == first_model:
            return None
    ...
    def _budget_exhausted(self, start_wall: float) -> bool:
        """
        True when a configured request-timeout budget has been consumed.
        A budget of 0 (default) means no deadline.
        """
        budget = settings.request_timeout_budget_seconds
        ...
        return (time.perf_counter() - start_wall) >= budget
        ...
        **generation_kwargs: Any,
        ...
        for provider, model in candidates:
            ...
            while retry_no <= max_retries:
                ...
                if self._budget_exhausted(start_wall):
                    break
