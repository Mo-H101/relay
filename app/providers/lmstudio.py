...
# (``app.providers.factory``) resolve LM Studio providers straight to
# ``app.providers.lmstudio_client`` and ``build_runtime_provider``. The only
...
def create_provider() -> Provider:
    ...
    registry is the single source of runtime truth. LM Studio is a local
    OpenAI-compatible endpoint and does not require an API key, so model
    discovery runs regardless of key configuration; on discovery failure
    the provider is still returned (with no models) so it remains
    registered rather than crashing Relay startup.
    """
    return build_runtime_provider(PROVIDER_REGISTRY["lmstudio"])
